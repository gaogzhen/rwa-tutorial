...
/**
 * RealEstateTokenRetry 部署模块（带重试功能）
 * 部署到 Avalanche Fuji 测试网
 * 
 * 此合约使用 RealEstatePriceDetailsRetry，JavaScript 代码中包含 API 请求重试功能
 * 
 * 部署前需要设置以下配置变量：
 ...
 * 
 * 部署命令：
 * npx hardhat ignition deploy ignition/modules/RealEstateTokenRetry.ts --network avalancheFuji
 * 
 * 注意：部署前请确保：
 ...
 */
export default buildModule("RealEstateTokenRetryModule", (m) => {
  // 部署参数配置
  ...
  const ccipRouterAddress = m.getParameter(
    "ccipRouterAddress",
    "0xF694E193200268f9a4868e4Aa017A0118C9a8177" // Avalanche Fuji CCIP Router 地址
  );
  ...
  // 部署 RealEstateTokenRetry 合约
  const realEstateTokenRetry = m.contract("RealEstateTokenRetry", [
    baseURI, // 基础 URI
  ...
  return { realEstateTokenRetry }; // 返回部署的合约实例
});
